...
以手机实拍的外观/交互视频为唯一依据，对 32 个日常物体（家具、电器、数码外设、包装软物、厨房餐具）进行**外观与交互的一对一重建**：为每项构建带原生 USD Physics 关节的可仿真 USD 资产，在 Newton（SolverMuJoCo）中复现真实交互动作，并产出逐视角的外观对照图与左右并置的交互对比视频。
